...
#define OPTIGA_TRUST_X_KEY_SLOT_STATIC              OPTIGA_KEY_STORE_ID_E0F1
#define OPTIGA_TRUST_X_KEY_SLOT_EPHEMERAL           OPTIGA_KEY_STORE_ID_E0F2
...
{
    OCKAM_ERR ret_val = OCKAM_ERR_NONE;
    OPTIGA_TRUST_X_PEER_PUBLIC_KEY_s *p_peer_pub_key = 0;
    ...
    do
    {
        ...
        if(key_type == OCKAM_VAULT_KEY_STATIC) {                /* Get the static public key                          */
            p_peer_pub_key = &g_optiga_trust_x_pub_keys[OPTIGA_TRUST_X_PUB_KEY_STATIC];
        } else if(key_type == OCKAM_VAULT_KEY_EPHEMERAL) {      /* Get the generated ephemeral public key             */
            p_peer_pub_key = &g_optiga_trust_x_pub_keys[OPTIGA_TRUST_X_PUB_KEY_EPHEMERAL];
        } else {
            ret_val = OCKAM_ERR_INVALID_PARAM;
            break;
        }

        if(p_peer_pub_key->remaining_length == 0) {             /* Ensure the key has been initialized                */
            ret_val = OCKAM_ERR_VAULT_TPM_KEY_FAIL;
            break;
        }

        ret_val = ockam_mem_copy(p_pub_key,
                                 p_peer_pub_key->public_key,
                                 pub_key_size);
                               ...
                               uint8_t *p_pms, uint32_t pms_size)
{
    ...
    do {
        ...
        peer_pub_key.remaining_length   = 0x42;                 /* Remaining length                                   */
        peer_pub_key.reserved_0         = 0x00;                 /* Unused bits                                        */
        peer_pub_key.compression_format = 0x04;                 /* Compression format                                 */

        ockam_mem_copy(&(peer_pub_key.public_key),
                       p_pub_key,
                       pub_key_size);
